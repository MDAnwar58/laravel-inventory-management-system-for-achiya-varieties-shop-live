...
@section('content')
  <x-admin.breadcrumb :breadcrumbs="$breadcrumbs" :create_btn_name="'Create'" :btn_route="'admin.category.create'" />
...
@push('script')
  ...
  <script>
    ...
    function getData(searchVal, filterVal, sortCol, sortType, currentPage, perPageVal) {
        ...
        $.ajax({
            url: `${baseUrl}/admin/categories/get`,
            method: 'GET',
            data: {
                search: searchVal,
                filter: filterVal,
        ...
        if (loading) {
            ...
            datas.forEach((data, index) => {
                html += `<tr>
                    ...
                    <td>${data?.item_type?.name || ''}</td>
                    <td>${data.status === 'active' ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Deactive</span>'}</td>
                    <td>
                        <div class="d-flex gap-1 align-items-center">
                            <a href="/admin/category/edit/${data.id}" class="btn btn-sm btn-outline-primary pt-2 pb-1 rounded-3">
                                <i class="fa-solid fa-pen-to-square fs-5"></i>
                            </a>
                            <button type="button" data-url="/admin/category/delete/${data.id}" class="btn btn-sm btn-outline-danger delete-btn pt-2 pb-1 rounded-3 ${deleteOptionIsLock ? 'disabled' : ''}">
        ...
        $.get(baseUrl + url)
            .done(function(res) {
                if (res.status === 'success') {
                    ...
                    console.error('error:', "Wrong Way! Please try again.");
                }
            })
            .fail(function(err) {
                console.error(err);
