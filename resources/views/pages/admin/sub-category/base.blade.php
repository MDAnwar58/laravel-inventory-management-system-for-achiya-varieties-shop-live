...
@section('content')
  <x-admin.breadcrumb :breadcrumbs="$breadcrumbs" :create_btn_name="'Create'" :btn_route="'admin.sub.category.create'" />
  <x-admin.page-title title="Sub Category" />
...
@push('script')
  ...
  <script>
    ...
    function getData(searchVal, filterVal, sortCol, sortType, currentPage, perPageVal) {
        ...
        $.ajax({
            url: `${baseUrl}/admin/sub-categories/get`,
            method: 'GET',
            data: {
                search: searchVal,
                filter: filterVal,
    ...
    function renderTable() {
        let html = '';
        if (loading) {
            html = `<tr><td colspan="7" class="text-center fs-4 py-3 fw-semibold">Loading...</td></tr>`;
        } else if (datas.length > 0) {
            datas.forEach((data, index) => {
                html += `<tr>
                    <td>#${index + 1}</td>
                    <td>${data.image ? `<img src="${data.image}" alt="${data.name}" style="width:50px; height:50px; object-fit:cover; border-radius:5px;">` : 'N/A'}</td>
                    ...
                    <td>
                        <div class="d-flex gap-1 align-items-center">
                            <a href="/admin/sub-category/edit/${data.id}" class="btn btn-sm btn-outline-primary pt-2 pb-1 rounded-3">
                                <i class="fa-solid fa-pen-to-square fs-5"></i>
                            </a>
                            <button type="button" data-url="/admin/sub-category/delete/${data.id}" class="btn btn-sm btn-outline-danger delete-btn pt-2 pb-1 rounded-3 ${deleteOptionIsLock ? 'disabled' : ''}">
            ...
            html = `<tr><td colspan="7" class="text-center fs-4">Data Not found.</td></tr>`;
        }
    ...
    function renderPagination() {
        ...
        links.forEach(link => {
            ...
            if (link.label === '&laquo; Previous') {
                $pagination.append(`<li class="${liClass}"><button class="page-link prev-btn" style="cursor:pointer;">${link.label}</button></li>`);
            } else if (link.label === 'Next &raquo;') {
                $pagination.append(`<li class="${liClass}"><button class="page-link next-btn" style="cursor:pointer;">${link.label}</button></li>`);
            } else if (link.label === '...') {
                $pagination.append(`<li class="${liClass}"><button class="page-link" style="cursor:pointer;">...</button></li>`);
            } else {
                $pagination.append(`<li class="${liClass}"><button class="page-link page_btn" data-page="${link.label}" style="cursor:pointer;">${link.label}</button></li>`);
            }
        });
    }
        ...
        $.get(baseUrl + url)
            .done(function(res) {
                if (res.status === 'success') {
                    ...
                    processNotify(res.msg, res.status); // Replace with your notification system
                    page = p;
                    getData(search, filter, sortColumn, sort, page, perPage);
                } else console.error('error:', "Wrong Way! Please try again.");
            })
            .fail(function(err) {
                console.error(err);
