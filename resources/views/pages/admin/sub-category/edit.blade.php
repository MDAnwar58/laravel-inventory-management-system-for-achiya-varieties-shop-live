    ...
    <div class="col-md-12">
      <div class="card shadow-sm">
        <div class="card-header pb-0">
          <h5 class="card-title mb-0 fs-4">Sub Category Edit</h5>
        </div>
        <form action="{{ route('admin.sub.category.update', $data->id) }}" method="POST" enctype="multipart/form-data"
          class="card-body">
          @csrf
          <div class="pb-3">
            <div class="d-flex align-items-center gap-1">
              <input type="text" name="name" class="px-3 py-2 fs-5 form-control  custom-input  input" placeholder="Name"
                value="{{ $data->name }}" />
            ...
            <div id="image-prev-div" class="d-flex {{ $data->image ? 'pb-2' : '' }}">
              <div class="position-relative">
                <button type="button" id="remove-btn"
                  class="btn btn-sm btn-outline-danger rounded-3 position-absolute top-0 end-0 me-3 mt-2 {{ $data->image ? '' : 'd-none' }}"><i
                    class="feather-sm" data-feather="x"></i></button>

                <img id="image-prev" src=" {{ $data->image }}" class="rounded-circle {{ $data->image ? '' : 'd-none' }}"
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      imageInput.addEventListener('change', function (e) {
        ...
        reader.onload = function (e) {
          ...
          imagePrevDiv.classList.add('pb-2')
        }
